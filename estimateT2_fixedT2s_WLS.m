function [T2_1_map, T2_2_map, S0_map, V1_map, mean_residual] = estimateT2_fixedT2s_WLS(images, TEs, mask)
        ...
        for k = 1:slices
            ...
            if all(signal > 0)
                if mask(i, j, k) > 0
                    ...
                    % Weights: higher weights for stronger signals
                    weights = signal .^ 2;
                    W = diag(weights);

                    % Residual function with weights applied (sqrt(W) * residual)
                    model_fun = @(p) sqrt(W) * ...
                        (p(1) * (p(2) * exp(-TEs / T2_1_fixed) + ...
                        (1 - p(2)) * exp(-TEs / T2_2_fixed)) - signal);

                    options = optimset('Display', 'off');
                    try
                        params = lsqnonlin(model_fun, p0, lb, ub, options);
...
for t = 1:num_echoes
    predicted = S0_map .* (V1_map .* exp(-TEs(t) ./ T2_1_fixed) + ...
                           (1 - V1_map) .* exp(-TEs(t) ./ T2_2_fixed));
    actual = images(:,:,:,t);
